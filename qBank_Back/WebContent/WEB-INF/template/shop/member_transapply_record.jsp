                    ...
                    <div class="marginB20 Padding20">
                        ...
                        <div>
                            <div class="PerProductListTitleNew">
                                ...
                                <div class="left w140 marginR10"><span>承接人姓名</span></div>
                                <div class="left w120 marginR10"><span>债券编号</span></div>
                                <div class="left w400 marginR10"><span>申请状态</span></div>
                            </div>
                            <div class="PerProductListNew">
                                <ul id="lists">
                  				<c:forEach items="${transferApplyList}" var="apply" varStatus="vs">
                                    <li>
                                        <div class="left w140 marginR30"><fmt:formatDate value="${apply.createDate}" pattern="yyyy-MM-dd" /></div>
                                        <div class="left w140">${apply.toUser.realName}
                                        </div>
                                        <div class="left w120 marginL10">${apply.zqNumber}</div>
                                        <div class="left w400 marginL10">
                                        	<c:choose>
                                        		<c:when test="${apply.transferStatus==0}">
                                        			审核中
                                        		</c:when>
                                        		<c:when test="${apply.transferStatus==1}">
                                        			审核通过
                                        		</c:when>
                                        		<c:when test="${apply.transferStatus==2}">
                                        			未通过
                                        		</c:when>
                                        		<c:otherwise>
                                        			${apply.transferStatus}
                                        		</c:otherwise>
                                        	</c:choose>
                                        </div>
                                    </li>
...
/* 			if("LI" == this.nodeName){
			...
			$.ajax({
				url: "/qbank/shop/member!transApplyAjaxList.action",
				data: param,
				dataType: "json",
				async: false,
					...
					if(data && data.list && data.list.length > 0){
						$.each(data.list, function(i, n){
							...
							htmlContent +=  '<li>'+
                                			...
                                			'<div class="left w140">'+n.creditorName+'</div>'+
                                			'<div class="left w120 marginL10">'+n.zqNumber+'</div>'+
                                			'<div class="left w400 marginL10">'+actionType+'</div>'+
                            				'</li>';
						})
					}else{
        ...
        jQuery("#nav").slide({
  		...
  		<script>
			$("#13").addClass("active").siblings().removeClass("active");
