...
<body class="list">
	...
	<div class="body">
		<form id="listForm" action="admin/article_type!list.action" method="post">
			<div class="listBar">
				<input type="button" class="formButton" onclick="location.href='admin/article_type!add.action'" value="添加"/>
				<label>查找:</label>
			...
			<table id="listTable" class="listTable">
				...
				<c:forEach items="${pager.list}" var="articleType" varStatus="vs">  
					<tr>
					<td></td>
						<td>
						...
						<td>
						<c:forEach items="${pager.list}" var="articleTypes" varStatus="vss"> 
						<c:if test="${ articleTypes.id==articleType.parentId}">${articleTypes.typeName}</c:if>
						</c:forEach>
						</td>
						<td>
						${articleType.artTypeCode }
						</td>
						<td>
						<fmt:formatDate value="${articleType.createDate }" type="both" dateStyle="long" timeStyle="short"/>	
						</td>
						<td>
							<a href="admin/article_type!edit.action?id=${articleType.id}" title="[编辑]">[编辑]</a>
							<a href="admin/article_type!delete.action?id=${articleType.id}" title="[删除]">[删除]</a>
						</td>
					</tr>
				</c:forEach>  	
