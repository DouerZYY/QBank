...
<title>会员列表 - Powered By ${setting.systemName}</title>
...
<body class="list">
	...
	<div class="body">
		<form id="listForm" action="admin/user!list.action" method="post">
			<div class="listBar">
				<input type="button" class="formButton" onclick="location.href='admin/user!add.action'" value="添加"/>
				<label>查找:</label>
			...
			<table id="listTable" class="listTable">
				...
				<c:forEach items="${pager.list}" var="user" varStatus="vs">  
					<tr>
					<td></td>
						<td>
						${user.name}
						...
						<td>
						<fmt:formatDate value="${user.createDate }" type="both" dateStyle="long" timeStyle="short"/>	
						</td>
						<td>
							<a href="admin/user!edit.action?id=${user.id}" title="[编辑]">[编辑]</a>
							<a href="admin/user!delete.action?id=${user.id}" title="[删除]">[删除]</a>
						</td>
					</tr>
